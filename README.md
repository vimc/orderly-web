...
1. If you want to manually test any functionality which requires interaction with the orderly server (e.g. publish or run report), 
    you will need the application to use the database in the `/git` subdirectory of `/src/app`, not the default `/demo` subdirectory. This will give you access to the git test data, which only has the 'minimal' report, not the full demo test data. 
