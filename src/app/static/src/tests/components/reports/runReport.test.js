...
describe("runReport", () => {
    ...
    it('shows new version', () => {
        ...
        expect(wrapper.find("#run-report-new-version a").attributes("href")).toBe("/report/name1/20190514-160954-fc295f38");
        expect(wrapper.find("#run-report-dismiss").text()).toBe("Dismiss");
    });
    ...
    it('posts run request when confirm run button is pressed', (done) => {
        ...
        setTimeout(()  => {
            ...
            expect(mockAxios.history.post[0].url).toBe("/report/name1/run/");
    ...
    it('updates status and stops polling when run update request returns success status', (done) => {
        ...
        mockAxios.onGet('/report/some_key/status/')
