...
class GetLatestReportVersionTests
{
    ...
    @Test
    fun `getLatestReportVersion uses display name if present`()
    {
        val sut = ReportController(mockActionContext, mockOrderly, mock(), mockReportRepo, mock())
        val result = sut.getLatestReportVersion()

        assertThat(result.report.displayName).isEqualTo("a fake report")
    }

    @Test
    fun `getLatestReportVersion uses name for display name if not present`()
    {
    ...
    fun `users with report running permissions are report runners`()
    {
        ...
        val sut = ReportController(actionContext, mockOrderly, mock(), mockReportRepo, mock())
        val result = sut.getLatestReportVersion()
    ...
    fun `creates correct breadcrumbs`()
    {
        ...
        val breadcrumbs = sut.getLatestReportVersion().breadcrumbs
