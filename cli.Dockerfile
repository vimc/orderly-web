...
ENV APP_DOCKER_PUBLIC_COMMIT_TAG $registry_public/$name:$git_id
ENV APP_DOCKER_PUBLIC_BRANCH_TAG $registry_public/$name:$git_branch
...
CMD ./gradlew :userCLI:test :userCLI:distDocker -i -Pdocker_version=$GIT_ID -Pdocker_name=$APP_DOCKER_TAG \
    ...
    && docker tag $APP_DOCKER_COMMIT_TAG $APP_DOCKER_PUBLIC_COMMIT_TAG \
    && docker tag $APP_DOCKER_COMMIT_TAG $APP_DOCKER_PUBLIC_BRANCH_TAG \
    && docker push $APP_DOCKER_BRANCH_TAG \
    && docker push $APP_DOCKER_PUBLIC_COMMIT_TAG \
    && docker push $APP_DOCKER_PUBLIC_BRANCH_TAG
